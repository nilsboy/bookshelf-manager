...
Manager.prototype.setModel = Promise.method(function(model, properties, options) {
  ...
  if (model.isNew() && properties && properties[model.idAttribute]) {
    promises.push(function() {
      return this.fetch(model, { [model.idAttribute]: properties[model.idAttribute] }, this.findRelated(properties), options).then(function(result) {
        if(!result) {
          throw new Error('No entry found in ' + model.tableName + ' with ' + model.idAttribute + ' = ' + properties[model.idAttribute])
        };
        return result;
      });
    }.bind(this));
...
Manager.prototype.setBelongsToMany = Promise.method(function(model, key, models, relation, options) {
  ...
  return Promise.resolve(existing.length ? existing : existing.fetch(options)).then(function() {
    ...
    return targets.mapThen(function(target) {
      if (!existing.findWhere({ [target.idAttribute]: target.id })) {
        return existing.attach(target, options);
      }
    }).then(function() {
      return existing.mapThen(function(target) {
        if (!targets.findWhere({ [target.idAttribute]: target.id })) {
...
Manager.prototype.setCollection = Promise.method(function(existing, models, options) {
  ...
  return Promise.map(models, function(properties) {
    var model = existing.findWhere({ [existing.idAttribute()]: properties[existing.idAttribute()] }) || existing.model.forge();
...
Manager.prototype.resetIdAttributes = Promise.method(function(model) {
  let that = this

  if(that.isCollection(model)) {
    model.forEach((entry) => { 
      that.resetIdAttributes(entry)
      return
    })
    return
  }
  ...
  Object.keys(model.relations).forEach(function(relationName) {
    ...
    that.resetIdAttributes(relation)
  })
})
